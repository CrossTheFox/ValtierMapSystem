...
function DetailPanel({ char, locationName, onClose, onWiki, wikiEntity }) {
    ...
    return (
        ...
        >
            <Box sx={{ px: 2, py: 1.25, borderBottom: `1px solid ${UI_COLORS.border}`, display: "flex", gap: 1.5, alignItems: "flex-start", flexShrink: 0 }}>
                <Box sx={{ flexShrink: 0 }}>
                    <CharAvatar imagePath={char.imageUrl} name={char.name} size={64} status={char.status} />
                </Box>
                <Box sx={{ flex: 1, minWidth: 0 }}>
                    <CyberTitle sx={{ fontSize: "13px", letterSpacing: "0.12em", color: UI_COLORS.accent }}>
                        {char.name?.toUpperCase()}
                    </CyberTitle>
                    <CyberText sx={{ fontSize: "11px", color: UI_COLORS.textSecondary }}>
                        {(char.type || "npc").toUpperCase()} · {locationName}
                    </CyberText>
                </Box>
            ...
            <Box sx={{ flex: 1, overflow: "auto", p: 2, minHeight: 0, ...CYBER_SCROLL_STYLE }}>
                ...
                {char.bio && (
                    <Box sx={{ mb: 2 }}>
                        ...
                        <CyberText sx={{ fontSize: "13px", color: "rgba(255,255,255,0.8)", lineHeight: 1.6 }}>
                            {char.bio}
                        </CyberText>
                    </Box>
                )}
                ...
                {char.stats && Object.keys(char.stats).length > 0 && (
                    <Box sx={{ mb: 2 }}>
                        ...
                        <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 0.75 }}>
                            {Object.entries(char.stats).slice(0, 6).map(([key, val]) => (
                                <Box
                                    key={key}
                                    sx={{
                                        display: "flex",
                                ...
                                >
                                    <CyberText sx={{ fontSize: "10px", color: UI_COLORS.textSecondary }}>{key.toUpperCase()}</CyberText>
                                    <CyberText sx={{ fontFamily: "'Fira Code', monospace", fontSize: "12px", color: UI_COLORS.anomaly }}>{val}</CyberText>
                                </Box>
                            ))}
                        </Box>
                    </Box>
    ...
    return (
        <CyberText
            sx={{
                ...
                alignItems: "center",
                gap: 1,
                "&::after": { content: '""', flex: 1, height: 1, bgcolor: UI_COLORS.border },
            }}
        >
            // {children}
        </CyberText>
        ...
        >
            <Box sx={{ flex: 1, display: "flex", minHeight: 0, overflow: "hidden" }}>
                ...
                <Box sx={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
                    <Box sx={{ px: 2, py: 1, borderBottom: `1px solid ${UI_COLORS.border}`, display: "flex", flexWrap: "wrap", gap: 0.75, alignItems: "center", flexShrink: 0 }}>
                        <CyberText sx={{ fontSize: "9px", color: UI_COLORS.textSecondary, letterSpacing: "0.1em", mr: 0.5 }}>FILTRAR</CyberText>
                        {TYPE_FILTERS.map((f) => (
                            <Chip
                                key={f}
                                label={f}
